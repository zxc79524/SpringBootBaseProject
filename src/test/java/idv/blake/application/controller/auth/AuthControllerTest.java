...
class AuthControllerTest extends BaseUnitTest {
	...
	public static String REFRESHO_TOKEN_URL = SecurityConfig.AUTH_URL + "/refresh";
...
//		getRefreshTokenIdFromJWT(responseEntity.getResultData().getRefreshToken() + "aaaa");
		...
		// 刷新Token
		result = mvc.perform(MockMvcRequestBuilders.post(REFRESHO_TOKEN_URL).accept(MediaType.APPLICATION_JSON)
		...
		// 測試錯誤的refresh token
		result = mvc
				.perform(MockMvcRequestBuilders.post(REFRESHO_TOKEN_URL).accept(MediaType.APPLICATION_JSON)
						.header("Content-TYPE", MediaType.APPLICATION_JSON)
		...
		// 測試token 是否正常驗證
		result = mvc.perform(
				MockMvcRequestBuilders.get(CHECK_TOKEN_URL)
